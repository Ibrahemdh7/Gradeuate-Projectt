...
export class ApiService {
  ...
  async addMenuItem(data){
    try {
      ...
      this.collection('menu').doc(data.restaurant_id).collection('allItems').doc(id).set(itemData)
      .then(() => {
        console.log(`Item ${id} has been successfully added to the menu for the restaurant ${data.restaurant_id}.`);
      })
      .catch((error) => {
        console.error(`Error adding item ${id} to the menu for the restaurant ${data.restaurant_id}: `, error);
      });
      return true;
    } catch (e) {
      console.error("Error in addMenuItem method: ", e);
      throw(e);
    }
  }
